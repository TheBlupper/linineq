...
from sage.all import ZZ, QQ, vector, matrix, identity_matrix, zero_matrix, block_matrix, xsrange, zero_vector, pari
...
def BKZ(
    ...
    if M.nrows() > M.ncols() or not transformation:
        L = M.BKZ()
        if transformation:
            warn("Retroactively (slowly) computing transformation matrix for BKZ "
                 "because fplll currently doesn't support this for matrices where "
                 "nrows > ncols, see https://github.com/fplll/fplll/issues/525")
...
def flatter(M, transformation: bool=False, path: str=_DEFAULT_FLATTER_PATH):
    ...
    if M.rank() < M.nrows():
        ...
        # TODO look into how slow mathnf is
        verbose(f'computing hnf of a {M.nrows()} x {M.ncols()} matrix', level=1)
        Mnew, R = pari(M.T).mathnf(flag=1).sage()
        Mnew, R = Mnew.T, R.T
        kerdim = M.nrows() - Mnew.nrows()
        M = Mnew
    else:
        kerdim = 0
